...
export const LESSON_STEPS: LessonStep[] = [
  ...
  // 2. Free Energy
  {
    ...
    content: null,
    subsections: [{ title: "Free Energy", content: <MathBlock>{'F = \\text{Energy} - \\text{Entropy}'}</MathBlock> }],
    config: p({ k: 0.1 }), setup: 'grid', symbols: [{symbol:"F", definition:"Free Energy", context:"Surprise"}],
    script: [ { at: 1, type: 'reset' }, { at: 20, type: 'force', targetId: 'all', vector: {x:0,y:2} }, { at: 50, type: 'annotate', label: "Minimizing Surprise", duration: 20 }, { at: 80, type: 'zoom', targetZoom: 1.2 } ],
  ...
  // 3. The Particle
  {
    ...
    content: null,
    subsections: [{ title: "State Vector", content: <MathBlock>{'\\psi_i = \\{ \\mathbf{r}_i, x_i, \\phi_i, s_i \\}'}</MathBlock> }],
    config: p({ couplingEnabled: false }), setup: 'grid', symbols: [],
    script: [ { at: 1, type: 'reset' }, { at: 20, type: 'zoom', targetZoom: 2.5 }, { at: 40, type: 'highlight', targetId: 'center', label: "Position r_i" }, { at: 60, type: 'pulse', targetId: 'center', label: "Activation x_i" }, { at: 80, type: 'annotate', targetId: 'center', label: "Spin s_i" } ],
  ...
  // 4. Activation Waves
  {
    ...
    content: null,
    subsections: [{ title: "Interference", content: <p>Constructive and Destructive Interference allows for "explaining away".</p> }],
    config: p({ phaseEnabled: true }), setup: 'fluid_flow', symbols: [],
    script: [ { at: 10, type: 'pulse', targetId: 'all', duration: 30 }, { at: 50, type: 'annotate', label: "Interference", duration: 10 }, { at: 80, type: 'force', targetId: 'all', vector: {x:1, y:0} } ],
  ...
  // 5. Synchronization
  {
    ...
    content: null,
    subsections: [{ title: "Binding", content: <p>Temporal binding through phase locking.</p> }],
    config: p({ phaseEnabled: true, couplingEnabled: true }), setup: 'kuramoto_sync', symbols: [],
    script: [ { at: 10, type: 'pulse', targetId: 'all' }, { at: 40, type: 'annotate', label: "Phase Locking" }, { at: 80, type: 'zoom', targetZoom: 1.5 } ],
  ...
  // 6. Intrinsic Spin
  {
    ...
    content: null,
    subsections: [{ title: "Spin", content: <MathBlock>{'M(s_i, s_j) = 1 + \\gamma s_i s_j'}</MathBlock> }],
    config: p({ spinEnabled: true }), setup: 'spin_cluster', symbols: [],
    script: [ { at: 10, type: 'zoom', targetZoom: 0.8 }, { at: 40, type: 'annotate', label: "Spin Channels" }, { at: 70, type: 'highlight', targetId: 'all' } ],
  ...
  // 8. Euclidean Metric
  {
    ...
    content: null,
    subsections: [{ title: "Metric", content: <MathBlock>{'d_{ij} = || \\mathbf{r}_i - \\mathbf{r}_j ||'}</MathBlock> }],
    config: p({ k: 0.1 }), setup: 'swarm', symbols: [],
  ...
  // 9. Vector Forces
  {
    ...
    content: null,
    subsections: [{ title: "Vectors", content: <p>Forces have direction, not just magnitude.</p> }],
    config: p({ k: 0.05 }), setup: 'fluid_flow', symbols: [],
    script: [ { at: 20, type: 'force', targetId: 'all', vector: {x:0, y:-1} }, { at: 60, type: 'annotate', label: "Directional Intent" } ],
  ...
  // 11. Hamiltonian
  {
    ...
    content: null,
    subsections: [{ title: "H", content: <MathBlock>{'H = T + V'}</MathBlock> }],
    config: p({ damping: 0.99 }), setup: 'swarm', symbols: [],
    script: [ { at: 10, type: 'spawn', targetId: 'all' }, { at: 60, type: 'annotate', label: "Energy Conservation" } ],
  ...
  // 12. Gradient Descent
  {
    title: "12. Gravity for Data",
    content: null,
    subsections: [{ title: "Gradient", content: <MathBlock>{'\\frac{dx}{dt} = -\\nabla F'}</MathBlock> }],
    config: p({ k: 0.08 }), setup: 'swarm', symbols: [],
    script: [ { at: 30, type: 'highlight', targetId: 'all' }, { at: 60, type: 'annotate', label: "Gravity" } ],
  ...
  // 13. Newton's Third Law
  {
    ...
    content: null,
    subsections: [{ title: "Action-Reaction", content: <MathBlock>{'F_{ij} = -F_{ji}'}</MathBlock> }],
    config: p({ k: 0.1 }), setup: 'spin_cluster', symbols: [],
    script: [ { at: 20, type: 'force', targetId: 'center' }, { at: 50, type: 'annotate', label: "Reciprocity" } ],
  ...
  // 14. Mean Field
  {
    title: "14. Consensus",
    content: null,
    subsections: [{ title: "Mean Field", content: <p>Particles poll their neighbors.</p> }],
    config: p({ couplingEnabled: true }), setup: 'grid', symbols: [],
    script: [ { at: 10, type: 'pulse', targetId: 'center' }, { at: 60, type: 'annotate', label: "Consensus" } ],
  ...
  // 15. Free Energy Principle
  {
    title: "15. The Principle",
    content: null,
    subsections: [{ title: "Optimization", content: <p>Life minimizes surprise.</p> }],
    config: p({ k: 0.1 }), setup: 'random', symbols: [],
    script: [ { at: 30, type: 'shake', targetId: 'all' }, { at: 70, type: 'annotate', label: "Minimizing..." } ],
  ...
  // 16. Morphogenesis
  {
    ...
    content: null,
    subsections: [{ title: "Growth", content: <p>Dynamic topology adjustment.</p> }],
    config: p({ r0: 60 }), setup: 'swarm', symbols: [],
    script: [ { at: 20, type: 'spawn', targetId: 'all' }, { at: 60, type: 'annotate', label: "Adaptation" } ],
  ...
  // 17. Logic Gates
  {
    title: "17. Physics Logic",
    content: null,
    subsections: [{ title: "Gates", content: <p>AND, OR, NOT gates built from potential wells.</p> }],
    config: p({ k: 0.1, spinEnabled: true }), setup: 'logic_gate', symbols: [],
    script: [ { at: 10, type: 'highlight', targetId: 'all' }, { at: 50, type: 'annotate', label: "AND Gate" } ],
  ...
  // 18. Recurrence
  {
    title: "18. Working Memory",
    content: null,
    subsections: [{ title: "Loops", content: <p>Dynamic Attractors maintain state.</p> }],
    config: p({ damping: 0.99 }), setup: 'memory_loop', symbols: [],
    script: [ { at: 10, type: 'force', targetId: 'all', vector: {x:2,y:0} }, { at: 60, type: 'annotate', label: "Recurrence" } ],
  ...
  // 19. Superposition
  {
    ...
    content: null,
    subsections: [{ title: "Summation", content: <p>Waves sum instantly.</p> }],
    config: p({ phaseEnabled: true }), setup: 'kuramoto_sync', symbols: [],
    script: [ { at: 30, type: 'pulse', targetId: 'all' }, { at: 70, type: 'annotate', label: "Analog Math" } ],
  ...
  // 20. Inhibition
  {
    title: "20. The Veto",
    content: null,
    subsections: [{ title: "Inhibition", content: <p>Feedforward inhibition as a safety brake.</p> }],
    config: p({ spinEnabled: true }), setup: 'inhibitory_circuit', symbols: [],
    script: [ { at: 20, type: 'force', targetId: 'center' }, { at: 60, type: 'annotate', label: "VETO" } ],
  ...
  {
    title: "21. Temperature",
    content: null,
    subsections: [{ title: "Annealing", content: <p>Escaping local minima via chaos.</p> }],
    config: p({ temperature: 2.0 }), setup: 'swarm', symbols: [],
    script: [ { at: 10, type: 'shake', targetId: 'all' }, { at: 60, type: 'annotate', label: "Escaping..." } ],
  ...
  // 22. Rotational Invariance
  {
    title: "22. Symmetry",
    content: null,
    subsections: [{ title: "Invariance", content: <p>Geometry is independent of orientation.</p> }],
    config: p({ k: 0.1 }), setup: 'grid', symbols: [],
    script: [ { at: 30, type: 'highlight', targetId: 'all' }, { at: 70, type: 'annotate', label: "Invariant" } ],
  ...
  {
    title: "23. De Broglie Wavelength",
    content: null,
    subsections: [{ title: "Wavelength", content: <MathBlock>{'\\lambda = h / p'}</MathBlock> }],
    config: p({ couplingEnabled: true }), setup: 'grid', symbols: [],
    script: [ { at: 10, type: 'zoom', targetZoom: 1.5 }, { at: 60, type: 'annotate', label: "Variable Attention" } ],
  ...
  // 24. Probability Current
  {
    title: "24. Cognitive Flow",
    content: null,
    subsections: [{ title: "Flow", content: <MathBlock>{'\\nabla \\cdot \\mathbf{J} + \\frac{\\partial \\rho}{\\partial t} = 0'}</MathBlock> }],
    config: p({ phaseEnabled: true }), setup: 'fluid_flow', symbols: [],
    script: [ { at: 20, type: 'pulse', targetId: 'all' }, { at: 60, type: 'annotate', label: "Thought Stream" } ],
  ...
  // 25. Adaptive Activation
  {
    ...
    script: [ 
        ...
        { at: 30, type: 'annotate', label: "Dilating (Gain UP)" },
        { at: 70, type: 'annotate', label: "Constricting (Gain DOWN)" }
    ],
    narration: "Biological neurons are not static switches. They are adaptive, living things. Think of your eye. In a dark room, your pupil dilates wide to capture every single stray photon, struggling to see in the gloom. In bright sunlight, it constricts to a pinprick to prevent you from being blinded by the glare. This is Homeostasis—the maintenance of stability. Our activation function does exactly this. It adapts its slope—its sensitivity—based on the local energy density. When the signal is faint and the environment is quiet, the function steepens. It increases its gain, amplifying whispers into shouts so that no information is lost. When the signal is loud and chaotic, it flattens. It decreases gain to prevent saturation, ensuring the neuron doesn't just max out and stop carrying information. Mathematically, this maximizes Fisher Information. It ensures the neuron is always operating in its most sensitive range, the 'sweet spot' of the curve, regardless of how loud or quiet the environment is. This allows the network to maintain optimal sensitivity across a massive dynamic range, distinguishing subtle patterns in the noise without getting blinded by the signal."
  },
  // 26. Criticality
  {
    title: "26. Edge of Chaos",
    content: null,
    subsections: [{ title: "Balance", content: <p>Between order and disorder.</p> }],
    config: p({ temperature: 0.5 }), setup: 'swarm', symbols: [],
    script: [ { at: 10, type: 'shake', targetId: 'all' }, { at: 60, type: 'annotate', label: "Critical State" } ],
  ...
  {
    title: "27. Top-Down Attention",
    content: null,
    subsections: [{ title: "Gravity", content: <p>Centripetal force on outliers.</p> }],
    config: p({ k: 0.2 }), setup: 'grid', symbols: [],
    script: [ { at: 20, type: 'force', targetId: 'all', vector: {x:0, y:0} }, { at: 60, type: 'annotate', label: "Coherence" } ],
  ...
  // 28. CDMA
  {
    title: "28. CDMA",
    content: null,
    subsections: [{ title: "Multiplexing", content: <p>Orthogonal spin channels.</p> }],
    config: p({ spinEnabled: true }), setup: 'spin_cluster', symbols: [],
    script: [ { at: 20, type: 'highlight', targetId: 'all' }, { at: 60, type: 'annotate', label: "Parallel Processing" } ],
  ...
  // 29. Kuramoto Order
  {
    title: "29. Insight",
    content: null,
    subsections: [{ title: "Synchrony", content: <MathBlock>{'R = | \\frac{1}{N} \sum e^{i \\phi_j} |'}</MathBlock> }],
    config: p({ phaseEnabled: true, couplingEnabled: true }), setup: 'kuramoto_sync', symbols: [],
    script: [ { at: 30, type: 'pulse', targetId: 'all' }, { at: 70, type: 'annotate', label: "Aha!" } ],
  ...
  // 30. Nonlocal Elasticity
  {
    title: "30. Stiffness",
    content: null,
    subsections: [{ title: "Elasticity", content: <p>Instant communication.</p> }],
    config: p({ k: 0.3 }), setup: 'grid', symbols: [],
    script: [ { at: 10, type: 'force', targetId: 'center', vector: {x:10, y:0} }, { at: 40, type: 'annotate', label: "Shockwave" } ],
